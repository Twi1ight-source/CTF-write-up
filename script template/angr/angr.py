...
input = claripy.BVS("input", 8*34)    #len input
state = p.factory.blank_state(addr=0x40104d, add_options={angr.options.LAZY_SOLVES})

state.memory.store(0x412000, input)  #input stored at address 0x412000
state.regs.rdi = 0x412000

for i in range(34):                     #input must in printable
...
#6 ki tu dau bat dau bang 'DrgnS{'
